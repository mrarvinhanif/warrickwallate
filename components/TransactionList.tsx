...
const TransactionList: React.FC<TransactionListProps> = ({ transactions, onDelete, onEdit, currencySymbol, role }) => {
  return (
      ...
      <div className="max-h-[500px] overflow-y-auto px-4 pb-4 scrollbar-hide">
        {transactions.length === 0 ? (
          ...
          <div className="space-y-2 mt-4">
            {transactions.map((t) => (
              <div key={t.id} className={`group flex items-center justify-between p-4 rounded-3xl border transition-all duration-300 ${t.type === TransactionType.INCOME ? 'bg-green-100/60 border-green-200 hover:bg-green-200/80' : 'bg-red-100/60 border-red-200 hover:bg-red-200/80'}`}>
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${t.type === TransactionType.INCOME ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 relative z-20">
                    <button 
                      ...
                      className="p-2 text-blue-600 hover:text-blue-800 hover:bg-white/80 rounded-xl transition-all bg-white/40 shadow-sm border border-white/60"
                      title="Edit"
                    >
                    ...
                    <button 
                      ...
                      className="p-2 text-rose-600 hover:text-rose-800 hover:bg-white/80 rounded-xl transition-all bg-white/40 shadow-sm border border-white/60"
                      title="Delete"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
